...
async def approve_request(callback: CallbackQuery, is_admin: bool):
    ...
    async with async_session_factory() as session:
        ...
        name = request.requested_name or f"Подписка из шаблона {request.template.name}"
...
async def reject_request(callback: CallbackQuery, is_admin: bool):
    ...
    async with async_session_factory() as session:
        ...
        template_name = request.template.name if request.template else "Неизвестно"
        sub_name = request.requested_name or "Не указано"
        telegram_id = request.client.telegram_id
